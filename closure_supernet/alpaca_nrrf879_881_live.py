...
class AlpacaNRRF879881Runtime:
    ...
    def resolve_once(self) -> dict[str, Any]:
        ...
        return {
            ...
            "status": (
                trading.get("status")
                if bridge["anti_smuggling_audit"]["valid"] is True
                else "OPEN"
            ),
            "trading": trading,
            "nrrf879_881_runtime": bridge,
            "automatic_order_submission": False,
            "runtime_semantic_author_present": bridge["anti_smuggling_audit"][
                "runtime_semantic_author_present"
            ],
        }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run Alpaca through the NRRF879-881 formal runtime bridge"
    )
    ...
    runtime = AlpacaNRRF879881Runtime(
        AlpacaLiveClosureAdapter(AlpacaLiveConfig.from_env())
    )
    iteration = 0
    while True:
