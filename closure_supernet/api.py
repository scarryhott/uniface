...
from .models import AuthorDecision, Occurrence, OccurrenceCreate, RuleVersion, RuleVersionCreate, RuntimeCycleResult, RuntimeStatus
...
def create_app(config: RuntimeConfig | None = None) -> FastAPI:
    ...
    app = FastAPI(
        ...
        version="0.1.0",
        description="Source-preserving autonomous runtime for configured translational unification",
        lifespan=lifespan,
    )
    ...
    async def health() -> dict[str, Any]:
        return {"status": "ok", "runtime": runtime.status().model_dump(mode="json"), "database": str(runtime.config.database_path)}
    ...
    async def list_occurrences(limit: Annotated[int, Query(ge=1, le=5000)] = 500, offset: Annotated[int, Query(ge=0)] = 0) -> list[Occurrence]:
        return [Occurrence.model_validate(row) for row in runtime.store.list_occurrences(limit=limit, offset=offset)]
    ...
    async def candidate_relations(limit: Annotated[int, Query(ge=1, le=5000)] = 1000) -> list[dict[str, Any]]:
        return runtime.store.list_candidate_relations(limit)

    @app.get("/interpretations")
    async def interpretations(limit: Annotated[int, Query(ge=1, le=5000)] = 1000) -> list[dict[str, Any]]:
        return runtime.store.list_interpretations(limit)

    @app.get("/admissions")
    async def admissions(limit: Annotated[int, Query(ge=1, le=5000)] = 1000) -> list[dict[str, Any]]:
    ...
    async def author_decision(interpretation_id: str, data: AuthorDecision) -> dict[str, Any]:
        try:
            ...
            raise HTTPException(status_code=404, detail=str(exc)) from exc
        rule_version = f"author:{data.author_id}:{runtime.store.active_rule_version()}:{interpretation_id}:{data.verdict}"
        admission, _ = runtime.store.create_admission(
            interpretation_id, data.verdict,
            {"AUTHOR_DECISION": True, "SOURCE_REVERSIBLE": True, "REOPENING_AVAILABLE": True},
            data.reason, rule_version, f"author:{data.author_id}",
        )
        runtime.projection.run()
        return admission

    @app.get("/open-seams")
    async def open_seams(limit: Annotated[int, Query(ge=1, le=5000)] = 1000) -> list[dict[str, Any]]:
    ...
    @app.get("/events")
    async def events(after: Annotated[int, Query(ge=0)] = 0, limit: Annotated[int, Query(ge=1, le=5000)] = 500) -> list[dict[str, Any]]:
    ...
    async def bootstrap(root: Annotated[str | None, Body()] = None) -> dict[str, int]:
        count = await runtime.bootstrap_markdown(None if root is None else runtime.config.bootstrap_root / root)
    ...
    async def websocket_events(websocket: WebSocket) -> None:
        ...
        try:
            while True:
                events = runtime.store.events_after(after, 100)
                for event in events:
