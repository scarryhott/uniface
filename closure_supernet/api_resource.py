...
from . import api_translation as base_api
...
def attach_resource_routes(app: FastAPI) -> FastAPI:
    ...
    app.description += (
        ...
        "inside the canonical TranslationEvent field"
    )
    ...
    async def create_resource_translation(
        ...
        try:
            translation = await runtime.resource_protocol.create_translation(data)
            runtime.reconcile_resource_translations()
            return ResourceTranslation.model_validate(translation)
        except KeyError as exc:
            raise HTTPException(status_code=404, detail=str(exc)) from exc
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc
    ...
    async def decide_resource_translation(
        ...
        try:
            translation = runtime.resource_protocol.decide_translation(
                translation_id, data
            )
            runtime.reconcile_resource_translations()
            runtime.resource_protocol.integrate_live_stage(
                trigger="resource-translation-decision"
            )
            return ResourceTranslation.model_validate(translation)
        except KeyError as exc:
            raise HTTPException(status_code=404, detail=str(exc)) from exc
    ...
    async def reintegrate_resources() -> dict[str, Any]:
        ...
        return {
            ...
            "translation_field": runtime.translation_field(),
        }
