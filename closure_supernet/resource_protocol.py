...
class LiveResourceProtocolManager:
    """Live resource forms whose returns re-enter the translation field.

    Resource and engagement labels are open strings.  A wire/protocol verdict
    is recorded but never used as the truth verdict.  Natural components are
    generated only by participant-relative TRUE translation decisions.
    """
    ...
    def _validate_resource_refs(self, data: ResourceCreate) -> None:
    ...
    async def create_resource(self, data: ResourceCreate) -> dict[str, Any]:
        self._validate_resource_refs(data)
    ...
    async def create_engagement(
        ...
        source = self.store.get_resource(data.resource_id)
        ...
        occurrence = await self.ingest(
            OccurrenceCreate(
                ...
                source_context=f"Engagement '{data.engagement_label}' with {data.resource_id}",
                metadata={
                    **data.metadata,
                    "living_form": "RESOURCE_ENGAGEMENT",
                    "resource_id": data.resource_id,
                    "source_resource_occurrence_id": source["occurrence_id"],
    ...
    async def create_translation(
        ...
        occurrence = await self.ingest(
            OccurrenceCreate(
                ...
                source_context=f"Resource translation {source['id']} -> {target['id']}",
                metadata={
                    **data.metadata,
        ...
        retrieval_score = 0.5 if not values else sum(values) / len(values)
        candidate, _ = self.event_store.create_candidate_relation(
            source["occurrence_id"],
            target["occurrence_id"],
            str(RelationType.FRAME_TRANSLATION),
            max(0.0, min(1.0, retrieval_score)),
            (
                "An authored resource translation proposes mutual reading across "
                "relative forms; its transport verdict remains separate from truth"
            ),
    ...
    async def create_return(self, data: ResourceReturnCreate) -> dict[str, Any]:
        ...
        returned_resource = await self.create_resource(
            ResourceCreate(
                ...
                metadata={
                    **data.metadata,
                    "returned_from_engagement_id": engagement["id"],
            ...
            translation = await self.create_translation(
                ResourceTranslationCreate(
                    ...
                    exact_text=(
                        ...
                        "the relation is proposed without erasing either occurrence."
                    ),
                    ...
                    affected_perspectives=reintegration["affected_perspectives"],
    ...
    @staticmethod
    def natural_components(
        resources: list[dict[str, Any]], translations: list[dict[str, Any]]
    ) -> list[dict[str, Any]]:
        parent = {resource["id"]: resource["id"] for resource in resources}
        ...
        for translation in translations:
            if translation["current_verdict"] == str(Verdict.TRUE):
                union(translation["source_resource_id"], translation["target_resource_id"])

        groups: dict[str, list[dict[str, Any]]] = {}
        for resource in resources:
            groups.setdefault(find(resource["id"]), []).append(resource)
        ordered = sorted(groups.values(), key=lambda rows: min(row["id"] for row in rows))
        return [
            {
                "id": f"natural-component:{index}",
                "resource_ids": sorted(row["id"] for row in rows),
                "form_labels": sorted({row["form_label"] for row in rows}),
                "language_labels": sorted(
                    {row["language_label"] for row in rows if row["language_label"]}
                ),
                "occurrence_ids": sorted(row["occurrence_id"] for row in rows),
                "canonical_form": None,
    ...
    def canonical_limit_signature(
        ...
        admitted_pairs = sorted(
            sorted([row["source_resource_id"], row["target_resource_id"]])
            for row in translations
            if row["current_verdict"] == str(Verdict.TRUE)
        )
        return stable_hash(
            {
                "resource_occurrences": sorted(row["occurrence_id"] for row in resources),
    ...
    def integrate_live_stage(self, trigger: str = "manual") -> tuple[dict[str, Any], bool]:
        ...
        verdict_ids = {
            ...
            for verdict in (str(Verdict.TRUE), str(Verdict.OPEN), str(Verdict.FALSE))
        }
        components = self.natural_components(resources, translations)
        limit_signature = self.canonical_limit_signature(resources, translations)
        stage_signature = stable_hash(
            {
        ...
        reverse: dict[str, list[str]] = {
            f"resource:{row['id']}": [row["occurrence_id"]] for row in resources
        }
        for engagement in engagements:
            reverse[f"engagement:{engagement['id']}"] = [
                engagement["occurrence_id"],
                self.store.get_resource(engagement["resource_id"])["occurrence_id"],
            ]
        for translation in translations:
            reverse[f"resource-translation:{translation['id']}"] = [
                translation["occurrence_id"],
                self.store.get_resource(translation["source_resource_id"])["occurrence_id"],
                self.store.get_resource(translation["target_resource_id"])["occurrence_id"],
            ]
        for component in components:
            reverse[component["id"]] = component["occurrence_ids"]

        resource_keys = {f"resource:{row['id']}" for row in resources}
        stage = self.store.create_stage(
            {
                ...
                "admitted_translation_ids": verdict_ids[str(Verdict.TRUE)],
                "open_translation_ids": verdict_ids[str(Verdict.OPEN)],
                "rejected_translation_ids": verdict_ids[str(Verdict.FALSE)],
                "natural_components": components,
                "stage_signature": stage_signature,
                "limit_signature": limit_signature,
                "complete_coverage": resource_keys.issubset(set(reverse)),
                "canonical_language": None,
    ...
    def projection(self) -> dict[str, Any]:
        ...
        if current:
            reverse.update(current["source_reverse_index"])
        stats = {
            **self.store.stats(),
            ...
            "live_limit_matches_current_batch": current is None or current["limit_signature"] == batch_signature,
            "finite_resource_registry": False,
            "canonical_language_selected": False,
            "protocol_is_translational_truth": False,
            ...
            "complete_network_coverage": True if current is None else current["complete_coverage"],
            "nonterminal": True,
            "turing_complete_assumed": False,
        }
        return {
