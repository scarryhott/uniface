...
def _atlas_row() -> dict[str, object]:
    return {
...
def test_unsigned_caller_flags_cannot_author_current_truth(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    receipt = resolve_trading_equation(
        observer_id="o",
        sensor_feedback=[_row("a", "A", "B", "-3"), _row("b", "B", "A", "2")],
    )
...
def test_tampering_with_signed_relation_value_forces_open(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert receipt["status"] == "OPEN"
    assert receipt["natural_forms"] == []
    reasons = {row["reason"] for row in receipt["source_truth_audit"]["feedback"]["returns"]}
...
def test_nested_atlas_translation_is_verified_individually(monkeypatch: pytest.MonkeyPatch) -> None:
    private, authority = _authority(monkeypatch)
    raw = _atlas_row()
    unsigned_source = {
        "atlas_translations": [raw],
        "source_witness": {"caller": "outer-container-witness-must-not-count"},
    }
    unsigned, audit = verify_atlas_translation_sources(observer_id="o", sources=[unsigned_source])
    assert audit["nested_atlas_translations_verified_individually"] is True
    assert audit["outer_container_witness_cannot_author_nested_translation"] is True
    assert audit["verified_count"] == 0
    nested_unsigned = unsigned[0]["atlas_translations"][0]
    assert nested_unsigned["returned"] is False
    assert nested_unsigned["source_preserved"] is False
    assert "source_witness" not in unsigned[0]

    signed_row = dict(raw)
    signed_row["source_witness"] = issue_atlas_translation_witness(
        private_key=private,
        authority_id=authority,
        observer_id="o",
        source_event_id="exact-source:nested-atlas-1",
        source_stream="test-atlas-feed",
        row=raw,
    )
    verified, audit2 = verify_atlas_translation_sources(
        observer_id="o", sources=[{"atlas_translations": [signed_row]}]
    )
    assert audit2["verified_count"] == 1
    nested_verified = verified[0]["atlas_translations"][0]
    assert nested_verified["returned"] is True
    assert nested_verified["source_return_ids"] == ["exact-source:nested-atlas-1"]
